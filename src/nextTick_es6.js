...
const levels = [document.createElement("audio"), document.createElement("span")];
...
function runMesoLevelTask(e) {
  ...
  try {
    let task = this._mesoLevelTasks.shift();
...
function addMesoTasks(tasks, span) {
  ...
  var promise = new Promise(resolveImpl => resolve = resolveImpl);
  span._mesoLevelTasks = tasks.concat(resolve);//makes the _mesoLevelTasks immutable.
  for (let i = 0; i < span._mesoLevelTasks.length; i++)
...
export function macrotask(depth) {
  ...
  finalPromise.ticks = function (tasks) {
    ...
    if (!(tasks instanceof Array) || tasks.indexOf(fun => !(fun instanceof Function)) >= 0)
      throw new TypeError("'tasks' in 'macrotask.ticks(tasks)' must be an array of Functions.");
    if (!tasks.length)
      return true;
    const promise = addMesoTasks(tasks, span);
...
export function nextTick(cb) {
  ...
  let resolve;
  const promise = new Promise(resolveImpl => resolve = resolveImpl);
