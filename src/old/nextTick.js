(function () {
  ...
  function parseNextTickArg(cbs) {
    //todo why throw an error when the array is empty??
    //if (!cbs || cbs.length)
    //  return cbs??
  ...
  function addMesoLevelTasks(tasks, level) {
    tasks = parseNextTickArg(tasks); //todo I think this should be at the top..
    if(!tasks.length)
      return;
    var resolve;
...
//        grows when needed into [ audio, span , span , ... , span ]
  ...
  function getTopAudio(depth) {
    ...
    //var audio = top.children.length ? top.querySelector("audio") : top; //todo is this faster?? //todo I could also implement a TreeWalker..
    var audio = top;
...
//mesotask promises never fail.
  function nextMesoTicks(tasks, mesoLevels) {
    mesoLevels = Math.max(parseInt(mesoLevels), 0);   //todo implement this as macroTask(mesoLevels) which you can then call .nextTicks(tasks) on.
